...
export const setupSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.emailProvider === "gmail" && (value.smtpHost || value.smtpPort))
      ctx.addIssue({ code: "custom", message: "Gmail sets its own SMTP host and port" });
    if (value.emailProvider === "gmail" && value.emailFrom && !senderAddress(value.emailFrom))
      ctx.addIssue({ code: "custom", message: "Gmail emailFrom must include the account address" });
  });
export type Setup = z.infer<typeof setupSchema>;
function senderAddress(from: string) {
  return (
    /<([^<>\s]+@[^<>\s]+)>\s*$/.exec(from)?.[1] ?? /^([^<>\s]+@[^<>\s]+)$/.exec(from.trim())?.[1]
  )?.toLowerCase();
}
const volumeName = "civic_spark_data";
const quote = (value: string) => JSON.stringify(value);
export function flyEnv(input: Setup) {
  ...
  if (input.emailProvider === "gmail")
    Object.assign(env, { SMTP_HOST: "smtp.gmail.com", SMTP_PORT: "587", SMTP_SECURE: "false" });
  return env;
}
...
export function emailSecrets(input: Setup, secrets: Record<string, string>) {
  ...
  for (const name of names) {
    ...
    if (!value || /[\r\n\0]/.test(value)) throw new SetupError(`Missing or multiline ${name}`);
    values[name] = value;
  }
  if (input.emailProvider === "gmail") {
    const password = (values.SMTP_PASSWORD ?? "").replace(/ /g, "");
    if (!/^[a-z]{16}$/i.test(password))
      throw new SetupError("Gmail needs a 16-letter app password, not the account password");
    if (values.SMTP_USER?.toLowerCase() !== senderAddress(input.emailFrom ?? ""))
      throw new SetupError("Gmail SMTP_USER must be the emailFrom address");
...
export function secretInput(input: Setup, secrets: Record<string, string>) {
  ...
  const envelope = Buffer.from(
    JSON.stringify({ ...secrets, ...emailSecrets(input, secrets) }),
  ).toString("base64");
...
async function main() {
  ...
  if (
    !action ||
    !path ||
    flags.some((flag) => flag !== "--ambient") ||
    (action === "test-email" && !z.email().safeParse(recipient).success)
  )
    throw new SetupError(
