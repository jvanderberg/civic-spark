...
- [ ] Push workspace status changes over a per-workspace events socket instead of fixed polls and cap the `changes` payload; re-run fifty with relay workers on and attribute loop time per process.
