...
- [x] Backup ZIP downloads with more than 65,535 entries use ZIP64 end records (the live download failed with "too many files"); the upload reader follows them. Checked with Info-ZIP `unzip -t` and macOS `ditto`
- [x] Backup downloads stream straight from the stored archive with per-file checksum checks: no unpack step or backup lock, so they start at once and never block or get blocked by other backup actions; only deleting that backup waits for its downloads
...
- [x] Scenario `agent` field (`opencode` or `claude`) and roster `--agent`; a Claude canary passed end to end on v45 (12m50s, real data with sources). Seventh fifty-person attempt on v44 was a credit outage (22/50), not a platform result; no running participant was idle-paused.
...
- [x] Agent chat composer defects: attachments clear with the text the moment a message is handed over and return only if the send fails; Stop is acknowledged by the server before the runner settles, drops the stopped turn's later output from transcript and replay, and wakes the OpenCode reconciler so the native abort is issued immediately while the Sprite activity hold is still kept until the turn settles; messages sent during a turn wait with the runner, survive a reload, can be taken back and are delivered once each; a spent provider balance is reported as a billing failure that keeps the saved key connected and the composer usable.
