...
- For UI changes, test the changed flows at phone and desktop sizes in both system themes. Inspect actual browser screenshots and console errors; if that is impossible, state the verification limits. Viewport emulation does not prove physical-device keyboard or native-picker behavior.
- Browser checks: Playwright is not preinstalled in this Sprite. To take screenshots or run browser checks, install it once in the project with npm install --save-dev playwright and npx playwright install chromium (a few minutes, done once per Sprite), then write a small headless script that opens the running app at http://127.0.0.1:<port>, sets phone and desktop viewports and both color schemes, saves screenshots to a git-ignored folder, and logs console errors. Read the screenshots before reporting. If installation fails, say so and describe what remained unverified.
...
- To see what teammates have published, run civic-spark git fetch. It places the team branch at refs/civic-spark/team-incoming without changing your files; review it with git log HEAD..refs/civic-spark/team-incoming and git diff HEAD...refs/civic-spark/team-incoming, and read a teammate's version of a file with git show refs/civic-spark/team-incoming:<path>. When the participant asks you to bring in team changes or resolve conflicts with them, merge that ref locally, resolve, and commit; fetch first so both sides are present. Publishing still requires confirmation and civic-spark git publish.
