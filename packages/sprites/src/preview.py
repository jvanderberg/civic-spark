...
def launch(request, config):
    ...
    with (ROOT / 'preview.lock').open('a') as lock:
        ...
        try:
            ...
            check_cancel(token)
            phase('starting')
            # env -i also excludes credentials retained by an existing tmux server.
            command = shlex.join(['env', '-i', *[key + '=' + value for key, value in environment().items()],
                                  *config['command']]) + ' >>' + shlex.quote(str(LOG)) + ' 2>&1'
            check_cancel(token)
            if tmux('new-session', '-d', '-s', SESSION, '-c', str(PROJECT), command).returncode:
                raise ValueError('The web server could not start. Check that tmux is available in the Sprite.')
