...
Checkpoint: September 16, 2026, America/Chicago. Rename, admin project briefs and the Fly deployment package are integrated. Live deployment remains pending.
...
- The [Fly package](fly-deployment.md) provides a pinned container, single Machine/volume setup, explicit or ambient CLI authentication, secure service credential staging, production HTTPS/email guards and a trusted proxy boundary. Setup is repeatable and does not hardcode an owner organization. No live deployment was performed.
...
1. **Prepare the first hosted installation.** Follow [Fly deployment](fly-deployment.md): choose app/org/region/origin, verify proxy peers and email sender, obtain a dedicated Sprite organization token, and keep the complete credential input outside the repository. Run plan/auth and the documented verification before cloud provisioning, secret staging and deployment. Linux container build/startup, actual volume recovery, email delivery, Sprite lifecycle and token rotation are not yet live-verified.
