...
2. Sign in as a site owner and create an event from a blank or DIOD template. The creator becomes its first admin. Only owners listed in the installation's setup can create events.
...
- Hidden files and directories are ordinary project data: `.gitignore`, `.github/`, `.vscode/`, `.editorconfig`, lint and format configuration and similar appear in the editor, Changes and sync and are shared. Only private or machine state is excluded: `.env` files other than `.env.example`, package-manager and shell credentials (`.npmrc`, `.netrc` and similar), SSH, cloud and container credentials, agent state (`.civic-spark-agent`, `.claude`, `.opencode`, `.codex`), the folder-sync marker, `.DS_Store`, caches, `credentials/` and `secrets/` directories, and key, certificate and log files. A refused Share names the excluded paths so they can be removed. Changes follows Git's ignore rules: untracked files that the project ignores, such as build output and caches, are not listed, do not move the preview fingerprint, and are never staged by Share; a committed file that later matches a rule still counts as a change. Other hidden paths, credentials and links remain excluded. Share checks every incoming commit and preserves the exact existing native commit when there are no additional edits; an ignore-rule change does not require rewriting history.
- **Replace the team repository with this workspace** in Changes, and **Use my version** in the team-updates overlap menu, are the same typed-confirmation escape hatch for a stuck team. It commits the shown changes, then sets the team version to that commit even if teammates published first. The displaced team version stays in the repository as a dated backup ref and the activity feed records who replaced it; teammates are asked to update or replace their copies as usual.
...
Event admins can create, download, upload, restore and delete installation backups from **Admin → Backups** while the application runs; see [admin console backups](backup-restore.md#admin-console-backups). Archives are deliberately unencrypted so there is no key to manage; they carry no session tokens or signing secrets, but account emails and event content are readable by anyone holding a download. Live capture snapshots each store consistently without stopping participants. Restore accepts any Civic Spark backup made in the same sign-in mode (other installations produce a warning), stages a recovered root, restarts the application to swap it in, signs everyone out, keeps newer permanent preview origins, reports Sprites reserved or released since the backup, and retains the replaced data beside the restored copy. It never calls the provider, publishes Git or replays agent work.
