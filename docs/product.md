...
Provider limits remain separate from event capacity. Preparation distinguishes the provider's documented concurrent-workspace limit from creation request throttling, authentication and service failures. A sanitized initial creation-failure category survives retries and management restarts until preparation succeeds. Unknown historical failures remain unknown. Retrying checks the same reserved workspace; an absent or uncertain reservation is never replaced automatically. A confirmed missing reservation directs the participant to an event admin for investigation. These error-handling changes are locally implemented; deployed acceptance remains root-owned. See [provider error contracts](provisioning-errors.md).
