...
VibeHack uses email links for both signup and returning sign-in. Enter an email address and an optional name, receive a link, and open it to verify the address and sign in. Links expire after 10 minutes and work once. A returning user gets the same account and memberships. No password or Google/Apple app registration is needed.
...
| Service | Free allowance | VibeHack adapter |
| --- | --- | --- |
...
Fly's documentation recommends an external transactional sender such as Resend or Postmark. VibeHack uses Fly for the app, with outbound SMTP or HTTPS to the chosen sender; email delivery has no Fly-specific dependency. [Fly SMTP guidance](https://fly.io/docs/getting-started/troubleshooting/)
...
- Magic-link requests and verification are limited per IP/path, default 120 requests per minute, to accommodate attendees sharing Wi-Fi. Set `VIBEHACK_AUTH_REQUESTS_PER_MINUTE` to tune that limit. These request limits do not enforce a daily email budget.
