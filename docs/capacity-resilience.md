...
- Bound Sprite CLI command execution (default 16 active, 120 queued, ten-second queue wait), large transfer commands (two active), and accepted concurrent request-body reservations (256 MiB). Return explicit retryable busy errors; these constrain transient work, not eligible people or allocated workspaces. Matching concurrent read polls coalesce by Sprite/script/payload, and read-only helper polls travel over one persistent per-Sprite helper session (one JSON line each way) instead of a CLI process per poll; the session is passive for idle accounting, bounded per reply, and falls back to a one-shot command when unavailable. Long preview preparation still competes for command slots.
...
## Relay worker processes

The fifty-person run on one vCPU showed the management event loop at 68 % median and 96 % peak CPU with 147 child processes and 536 sockets: every byte from every agent runner, terminal PTY and helper session was read, parsed, validated and re-sent by the same loop that served HTTP, so first byte rose to 7–11 s while handlers stayed near 100 ms. Cluster mode cannot help because sessions and leases are per-workspace memory. The relay moves the children out instead.

- The server forks a fixed pool of relay workers (`CIVIC_SPARK_RELAY_WORKERS`, default 2, at most 16) with `child_process.fork` and advanced serialization. Each workspace maps to one worker by a stable SHA-256 hash of its id, so its agent runner, terminal, helper session and one-shot commands share a process. Workers inherit the server environment and launch only the fixed `sprite` CLI; no participant code runs on the host.
- Workers own spawning, stdout line reading and validation, agent replay bookkeeping and 50 ms text-delta merging, the 200 KB terminal history ring and 16 ms output coalescing, helper-session multiplexing with its line and in-flight limits, and stderr suppression. They emit `agent.runner` and a ten-second `relay` telemetry record (loop delay percentiles, CPU share, RSS, owned sessions) themselves, so per-process attribution survives.
- The main process keeps HTTP, the WebSocket upgrade with Origin, ownership and execution checks, the periodic authorization re-check, browser input validation, per-client fan-out with the existing `bufferedAmount` close (1013), the command queue (`CIVIC_SPARK_MAX_COMMANDS`, transfer slots, read coalescing), `sprite.command`/`sprite.operation`/`ws` diagnostics, leases, `hasActiveWork`, idle release and pause/delete drains. Idle accounting is unchanged: terminal output is not use, agent user/text/tool/done events are use (throttled to once a second in the worker), helper sessions stay passive.
- The channel carries few, compact messages: agent frames batched per session per tick, one terminal frame per tick (merged further, newest bytes kept within 1 MiB per session, while the channel reports congestion), replay on attach correlated by client id so a client joins live fan-out only after its replay, `busy` transitions instead of per-event state, and command results as one buffer. Prompts, typed input and provider keys travel exactly as they did to the runner's stdin; failed commands carry only exit code, signal, kill flag and stderr bounded to 64 KiB for classification.
- When a worker exits, the main process closes its agent sockets with 1011 "Agent runner ended; reconnect" and terminal sockets with 1000 "Terminal detached; reconnect to resume", releases the leases, fails in-flight commands as ordinary command failures (helper reads fall back once to a one-shot command), logs `relay.worker` with exit code or signal, and forks a replacement after one second, doubling to at most 30 s only for rapid repeated crashes. Messages sent while a worker restarts wait for it. Shutdown asks each worker to stop its children and waits up to six seconds before killing it.
- `CIVIC_SPARK_RELAY_WORKERS=0` runs every child in the server process exactly as before; the in-process backends are the same code the workers use. The test suite runs in that mode by default, and `tests/relay-workers.test.ts` runs real worker processes with a fake `sprite` executable through agent turns and reconnect replay, terminal history reattach, helper sessions, a SIGKILLed worker with lease drain and reconnect, and shutdown; `tests/relay-worker-host.test.ts` and `tests/relay-pool.test.ts` cover batching, congestion merging, command and session relay, restart and diagnostics with fake children.

Unmeasured: no live Sprite run has exercised the relay yet, and the fifty-person re-run that would attribute loop time per process is the next step. One IPC channel per worker also carries large file transfers, so a 25 MiB result briefly delays that worker's agent frames; the main loop still copies those bytes once, as it did from the CLI pipe.
