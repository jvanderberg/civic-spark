...
- Replace per-workspace status timers with one change-notification WebSocket per open workspace tab (`/api/workspaces/:id/events`, same session/origin/execution checks and close codes as the agent and terminal sockets). Frames carry only a scope (`files`, `preview`, `agent-git`, `team`) and a timestamp; the browser then fetches that resource through the ordinary routes. The server publishes where it already knows something changed: agent tool steps (coalesced to one frame per 5 s) and finished turns, file saves and uploads, preview launches/stops from the browser or the agent CLI, conflict tickets, team updates and Share (the whole team hears `team`); lifecycle holds and generation changes close the sockets. Host-unobservable state (a teammate's push, a preview process ending inside the Sprite) is checked at most every 30 s per workspace, only while a tab is subscribed and execution is allowed, and pushed only on change; a stopped preview is not re-read. Tabs subscribe while visible, disconnect when hidden, reconnect with jittered bounded backoff, refresh on reconnect/return, and keep slow safety timers (files 90 s, preview/team/agent-git 120 s); without the channel the former timers apply. Estimated per visible person: idle drops from about 17 status requests (15 Sprite helper commands) per minute to about 3.5 requests (3 helper commands) per minute plus one host-side team-head ref read per 30 s; while the agent works, from about 52 requests per minute to at most one files refresh (three requests) per 5 s of tool activity plus one team check per finished turn. These are timer arithmetic, not a measured re-run.
