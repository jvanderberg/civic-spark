...
Event execution gates and per-workspace holds persist through EventService independently of provider status. `SpriteLifecycleProvider` supplies named-resource inspection and stop mechanics; its current HTTPS adapter uses `CIVIC_SPARK_SPRITE_API_URL`, while commands use the configured Sprite CLI organization. A second provider must implement non-destructive lifecycle operations and activity/wake semantics explicitly. Fly Machine stop is not a Sprite sleep operation. See [lifecycle contracts](sprite-lifecycle.md), including native idle and billing-data limits.
