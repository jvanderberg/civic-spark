...
For Fly-managed hostnames without DNS setup, the deployment helper can provision a bounded pool sized to the installation's `maxSprites`. Each ingress has one small, stateless Machine and no volume; the process runs only the trusted proxy in `apps/server/src/preview-ingress.ts`. Runtime `CIVIC_SPARK_PREVIEW_ORIGIN_POOL` is the helper-generated JSON array of HTTPS origins. `CIVIC_SPARK_PREVIEW_RELAY_SECRET` is a separate server-only credential shared by ingress and gateway, never the Sprite organization token or a Fly administrator token. Ingress uses a static configured HTTPS target and authenticated preview-origin metadata for HTTP and WebSocket relaying; browser-supplied relay and forwarding headers are stripped.
...
- Token rotation and revocation, limited allocation/concurrency, configured private preview opening, unauthorized-origin/session rejection, and documented backup/restore rehearsal. Live model inference requires separate authorization and is not part of this checklist.
