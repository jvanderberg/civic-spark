...
The explicit demo has passed live Linux startup, idle volume restart, public demo sign-in and dedicated Sprite workspace checks; see the live record below. This is not production certification: sender delivery, token rotation, full backup/restore and resource lifecycle recovery remain unverified. Hosted previews require the isolated HTTPS routing configuration below. Without it, Open preview fails explicitly; it never returns a Mac/loopback URL or makes a Sprite public. Remote Launch/Stop/status manage the server inside its Sprite. The demo now has an eight-origin Fly-managed ingress pool; dedicated live HTTP/assets/WebSocket, owner isolation, single-use ticket and logout-revocation checks passed. See the current handoff checkpoint for evidence and remaining limits.
...
## Private hosted previews

For an owned domain, set optional `previewOriginTemplate` in setup JSON, for example `https://{workspace}.preview.example.net`. Setup emits `CIVIC_SPARK_PREVIEW_ORIGIN_TEMPLATE`; this needs no frontend rebuild. The template must be an exact HTTPS origin with one `{workspace}` hostname label. It must use a different hostname from the management portal and cannot be beneath the portal hostname, so portal cookies cannot reach participant apps. Every workspace has a distinct browser origin, including its storage and service workers. Preview URLs are private by default; knowing the hostname does not authorize access.

For a domain the operator controls, point wildcard preview DNS at the existing Fly app and provision its wildcard certificate using DNS-01 validation. Route HTTPS and WebSocket traffic to the existing app HTTP service on port 4311. The application dispatches preview hosts before management API/auth routing; no additional Machine or volume is required. Wildcard DNS/TLS are operator prerequisites, not actions performed by the setup helper. Fly's default app-specific `fly.dev` certificate does not provide arbitrary child workspace hostnames. Follow [Fly custom domains](https://fly.io/docs/networking/custom-domain/).

Open preview first checks the signed-in workspace owner and actual server readiness. It creates a short-lived single-use handshake, redeemed on the preview origin for a Secure, host-only, HttpOnly cookie, then redirects to a clean URL. Each preview session retains the originating management session's authorization; opening another session does not revive older revoked sessions. HTTP requests and WebSocket connections recheck authorization. Browser credentials and proxy identity headers are stripped before tunneling to the Sprite. Participant programs continue to execute exclusively inside their existing Sprite, through authenticated standard Sprite transport; the organization token never reaches a browser or participant server.

The gateway forwards absolute-path assets and WebSocket upgrades, adapting Host/Origin for a loopback Vite server. It preserves existing app commands, files and process/session state. Management restart drops ephemeral preview sessions/tunnels; return to the workspace and select Open preview to obtain a new session and reattach the still-running Sprite server. This does not launch or restart the app. Expired or revoked previews also require reopening from an authorized workspace.

For Fly-managed hostnames without DNS setup, the deployment helper can provision a bounded pool sized to the installation's `previewPoolSize`. Each ingress has one small, stateless Machine and no volume; the process runs only the trusted proxy in `apps/server/src/preview-ingress.ts`. Runtime `CIVIC_SPARK_PREVIEW_ORIGIN_POOL` is the helper-generated JSON array of HTTPS origins. `CIVIC_SPARK_PREVIEW_RELAY_SECRET` is a separate server-only credential shared by ingress and gateway, never the Sprite organization token or a Fly administrator token. Ingress uses a static configured HTTPS target and authenticated preview-origin metadata for HTTP and WebSocket relaying; browser-supplied relay and forwarding headers are stripped.

An authorized Open preview automatically claims an unused pool origin and saves its workspace binding atomically in `preview-origins.json` inside the installation's auth-mode data directory. This routing file belongs in backups. Assignments survive management restarts and remain reserved after workspace removal. Capacity exhaustion fails explicitly; add pool capacity through the operator helper. Never delete bindings or reuse an origin for another participant, because browser storage and service workers can survive cookies. New teams need no DNS entry, origin editing or per-workspace provisioning command. The application has no Fly control-plane credentials and never creates ingress apps dynamically. Pool configuration and wildcard-template configuration are mutually exclusive.

Use `npm run test:hosted-preview-browser` after building for isolated TLS-edge and authenticated ingress relay, actual Vite asset/HMR and Open preview interaction coverage at 360px, 390px, desktop and short viewports in both themes. This uses a trusted synthetic fixture and mocked Sprite transport, no participant app or paid model. It does not establish physical-device support or live Fly routing. Keep proxy access logs from recording handshake query strings, cookies or authorization headers; the application does not log preview requests.
...
Set `previewIngress: true` in the retained private setup JSON. The helper generates a stable origin pool from `previewPoolSize`; participants never configure DNS or request per-workspace operator provisioning. Run `plan` after the management app has its original provisioning receipt, then:
...
The old `maxSprites` setup property is removed with no compatibility alias. Edit retained configuration explicitly; do not regenerate or discard the receipt, relay secret or permanent origin ledger. `previewPoolSize` is infrastructure capacity only and never limits Sprite allocation. Plan at least existing retained origins plus 60 for 60 additional membership workspaces. No-domain pools remain finite and require explicit append-only operator expansion; owned wildcard DNS/TLS avoids the per-origin ingress pool.
