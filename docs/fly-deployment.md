...
Rerun `npm run site:bootstrap <name>` to apply code or settings changes: it asks only for what is missing, keeps the auth secret, and skips the email test unless the sender or first owner changed. Edit `setup.json` there to change sizing or limits. Sign in as an owner with the emailed code and create the event. New events from the Day in Our Data template start with the Oak Park CISC starter briefs and project ideas. To add them to an event that already exists, run `npm run site:import-projects -- <name>`: it signs in as the first owner with an emailed code you type in, adds each project that isn't there yet, and signs out. `npm run site:import-projects -- template` regenerates the template from the source repository. The individual actions below remain for manual operation and for installations set up before this command.
...
Each site hosts one event. After deployment, an owner signs in and creates it; that event becomes the site's event, saved with the site's data, and no second event can be created. Optional `siteEventId` in setup JSON (`CIVIC_SPARK_SITE_EVENT_ID`) only adopts an existing event for installations that predate this, and only while no event has been adopted. Startup rejects malformed IDs or an event missing from that store instead of choosing another event. The session response supplies the event name for sign-in, navigation and the document title; anonymous visitors only receive names of registration/live events. Draft/closed names remain limited to authorized event members. A configured event unavailable to a signed-in visitor shows an explicit not-open message.
...
Provisioning reserves a full-UUID Sprite name and persists each phase before the external action. Startup marks interrupted jobs as retryable errors through the event service, without automatically creating resources or running models. Owner retry probes the same name and uses the existing non-destructive checkout script. Unknown provider outcomes never allocate another name. Deleted/revoked workspaces retain resources for recovery. There is no total allocation cap. `maxProvisioning` bounds concurrent setup calls; busy responses do not consume an allocation quota, and the participant app retries them automatically with backoff while showing that it is waiting for a slot. Agent preparation on a just-woken Sprite is retried up to three times server-side before it reports failure, and the app retries a failed preparation quietly once more before showing an error. `CIVIC_SPARK_MAX_COMMANDS` bounds transient Sprite CLI operations (default 16); large file operations have two transfer slots and matching concurrent polling reads coalesce. Read-only helper polls (file lists and reads, changes, manifest, preview status/logs, team status) share one long-lived `sprite exec` session per Sprite that runs a small dispatcher under `/home/sprite/.civic-spark-agent`; the session starts after the Sprite has answered a one-shot command, holds a passive lease so idle release still disconnects it, ends after two minutes without requests, and any failure falls back to the one-shot command once. Writes and uploads always use one-shot commands. `CIVIC_SPARK_HELPER_SESSIONS=0` disables sessions. These are backpressure controls, not a dollar budget, orphan-resource detector or account/workspace quota. No paid inference is used in checks.

The Sprite CLI children themselves (agent runners, terminal PTYs, helper sessions and one-shot commands) run in relay worker processes forked by the server, so their pipes and line parsing stay off the HTTP/WebSocket event loop; see [relay worker processes](capacity-resilience.md#relay-worker-processes). `relayWorkers` in setup JSON (default 2, range 0–16) becomes `CIVIC_SPARK_RELAY_WORKERS`. Set it to `0` and redeploy to run every child in the main process exactly as before the relay existed; that is the rollback for a live regression and needs no code change. Workers start only when Sprites are enabled, inherit the same environment (organization, token, diagnostics flag) and launch only the fixed `sprite` CLI. A worker that dies takes its sessions with it: affected browsers see the existing reconnect close codes, their leases are released, and the worker is restarted; `relay.worker` diagnostics record each start and exit.
