...
Local `npm run test:hosted-preview-browser` covers direct TLS, real Vite assets/HMR and Launch/Open at 360/390/desktop/short widths in both themes with provider doubles. It does not certify cold wake or physical devices. Root separately verified loopback-bound anonymous native HTTPS200, same-name service update, Stop producing no process/HTTP502, and explicit service start restoring HTTPS200 on one dedicated Sprite. Live Vite HMR and quiet hibernation/wake remain unverified. Root-owned dedicated acceptance must launch a disposable app, verify anonymous HTTPS/assets/HMR, verify ordinary no-traffic hibernation and later wake separately from explicit Stop; after Stop/Pause verify no process, then owner Launch recovery and management restart. Confirm same Sprite/URL, unchanged source/dependency cache, no portal cookies and no new Fly resources.
