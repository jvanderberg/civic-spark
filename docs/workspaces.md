...
Changes compares the working files with `refs/civic-spark/base`, initialized when the workspace is created and updated after a successful team sync. Existing workspaces fall back to their known `origin/main` or HEAD. New and deleted files are included; agent commits do not hide differences from the baseline. Binary files show a change notice. Text patches are bounded to 200,000 characters per file and 2 MB total; larger changes require terminal inspection. The local file adapter is TypeScript; the Sprite adapter runs a trusted Python script inside the VM. Neither diff path executes project hooks or external diff tools.
...
The top bar provides Launch/Restart, Open preview and compact diagnostics/Stop. Local previews use a separate loopback-hostname gateway; configured hosted previews use a distinct HTTPS origin per workspace. The gateway is owner-authorized, exchanges a short-lived single-use handshake for an expiring HttpOnly cookie, forwards assets and WebSockets, strips browser credentials before forwarding, and rechecks the originating session and workspace access. A top-level navigation uses SameSite=Lax so the initial cross-host redirect works in actual browsers; cross-origin API requests and WebSockets remain blocked. Hosted preview cookies are Secure and host-only. Preview content never executes on the control-plane host.

Hosted installations use an automatically assigned Fly ingress pool or set `CIVIC_SPARK_PREVIEW_ORIGIN_TEMPLATE` with owned wildcard DNS/TLS; see [private hosted previews](fly-deployment.md#private-hosted-previews). Preview hosts cannot serve management APIs/auth routes, and different workspaces do not share browser storage. Management restart requires Open preview again to reconnect to the existing app; it does not restart the participant process. Unconfigured installations fail explicitly. The gateway never switches a Sprite URL to public. The process/tunnel boundary remains portable; Fly private networking is not required. [Sprites networking](https://docs.sprites.dev/concepts/networking/), [managed services](https://docs.sprites.dev/concepts/services/).
