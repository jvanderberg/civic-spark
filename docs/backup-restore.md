...
**Download and upload.** Download streams the archive directory as a flat `.tar` for off-machine copies. Upload accepts only that layout, verifies the manifest checksum, and refuses archives made in a different sign-in mode. Uploads are stored as ordinary archives; nothing is restored by uploading.
