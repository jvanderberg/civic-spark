...
The [admin console](#admin-console-backups) can capture and restore the running installation. For coordinated offline capture and disaster recovery onto another host, `npm run backup -- create JOB.json` creates a versioned, authenticated encrypted directory artifact. `validate` decrypts into a private temporary root, verifies it, rehearses session invalidation and removes the temporary root. `restore` performs those checks and publishes an isolated, **startup-fenced** recovery directory. Neither restore command starts the application, executes backed-up source/configuration, contacts a provider, changes cloud resources, or overwrites an existing destination.

## Admin console backups

Event admins open **Admin → Backups** to create, list, download, upload, restore and delete backups of the whole installation without stopping it. Set `CIVIC_SPARK_BACKUP_OPERATORS` to a comma-separated list of account emails to limit these actions to named operators; otherwise any event admin on the installation may use them. Backups cover the complete data directory (every sign-in mode's SQLite stores, shared bare repositories, local prototype checkouts, preview-origin ledger, recovery evidence and unknown future files). Private Sprite files, saved model keys, agent conversations, process memory and deployment secrets are **not** included.

**Plain archives.** In-app backups are intentionally unencrypted: no key exists to manage or lose, and a downloaded file restores on any Civic Spark installation. They use the same directory layout as the CLI archives (`manifest.enc`, numbered blobs, `FINALIZED`) with a plain framing and per-file checksums, so `prune` retention applies to them; the CLI `restore` command does not read them and the console does not read CLI-encrypted archives. Because the files are readable, capture removes every session and login-link row from the copied auth store and never copies the local `auth-secret` signing file or the writer-lock database. Account names and emails, event content, memberships and shared Git history are readable by anyone holding a downloaded file; treat downloads accordingly.

**Live capture.** The running server copies each SQLite store with SQLite's online backup API and each Git repository with `git clone --mirror` (metadata such as `config`, hooks and reflogs is overlaid from the source), then verifies the copy with the same integrity checks as the CLI and seals it with `consistency: "live-online-snapshot"`. Each store is internally consistent at its own capture instant; there is no cross-store atomicity, and files that a local prototype workspace is writing at that instant may be captured mid-write. WAL/SHM sidecars and health probes are excluded. Staging happens only inside the private backup directory and is removed before the request returns. Capture needs free space for roughly twice the data directory.

**Storage.** Archives are stored in `CIVIC_SPARK_BACKUP_DIR` (default: a `-backups` sibling of the data directory, `/data/civic-spark-backups` on Fly) as `backup-<stamp>-<id>` directories. `installation.json` inside the data directory records a stable installation ID that every backup carries; `CIVIC_SPARK_RELEASE` (40-hex) records the deployed release, otherwise the checkout's `git rev-parse HEAD` or zeros.

**Download and upload.** Download unpacks the stored backup and streams it as an ordinary `.zip` (`civic-spark-backup-<stamp>/manifest.json`, `data/…`, `operator/…`) that opens with any archive tool; symbolic links are not exported and ZIP64 is not produced, so backups over 4 GB need the CLI path. Upload accepts such a ZIP (a shared top-level folder and Finder metadata are ignored), extracts it, drops any session rows, writer lock or auth-secret it may contain, verifies the tree with the same checks as capture and seals it as a stored archive under the original backup identity when `manifest.json` is present. Nothing is restored by uploading.

**Restore.** Restore replaces the whole installation with the selected backup. A backup from a different installation, release, origin or provider organization is allowed and reported as a warning in the confirmation result; only the sign-in mode must match. Confirming stages a recovered root beside the live data (`restore-<stamp>-<id>` in the backup directory) after unpacking, verifying the tree, revoking sessions and login links, keeping this host's `auth-secret`, and merging permanent preview origins allocated after the backup (never reassigning an existing binding). The report lists Sprites reserved after the backup (still running but untracked afterwards) and reservations released after it (they show as missing until owners rebuild from shared work). Nothing live changes until the application stops: the server responds, closes, applies the swap (live root renamed to `<data>.replaced-<stamp>`, staged root renamed into place), records `<data>.restore-last.json` and exits with code 3 on hosted deployments so the platform restarts it; local `npm start` exits 0 and must be started again. If the process is killed before the swap, the intent file `<data>.restore-pending.json` is applied at the next startup before any store opens. The replaced root is retained for operator inspection and is never deleted automatically. No provider call, checkpoint, Git push or model turn happens; there is no in-app undo other than restoring a later backup. A staged restore can be discarded from the console while the server is still running.

**Boundaries.** Restoring a backup that predates Sprite deletions or provider changes does not recreate resources; the existing owner-recovery flow applies, and a restore from another installation may reference Sprites this installation's token cannot reach. The encrypted CLI path below remains for operators who need a key-protected off-machine copy or the fenced reconciliation workflow. Hosted verification of the restart path and physical-device checks remain deployment-owner work.
...
The script validates the token's organization and requires an authenticated successful List Sprites response, including an empty list. The documented list does not require a top-level organization name; any returned top-level name or per-row organization claim must agree with the token. Each reservation receives a metadata GET: only exact 404 establishes missing; matching 200 means present; 401/403/429/5xx, redirects, malformed identity and network failures retain the fence. No participant files, bodies, keys or processes are read. The provider binding must match the encrypted backup's installation tuple. See [provider metadata API](https://docs.sprites.dev/api/v001-rc48/sprites/).
...
`tests/backup-live.test.ts` covers the admin API end to end with a real application in demo mode: live capture while the server is running (no sessions, auth secret or writer lock in the copy), listing, ZIP download/upload round trips (including archives produced by the system `zip` tool and unsafe-name rejection), cross-installation upload and restore warnings, operator allowlisting, staged restore with restart hook, the startup swap (sessions revoked, newer preview origins kept, untracked Sprites reported, replaced root retained, idempotent reapply), writer-lock protection, discard, and CLI restore of a live archive into a fenced root. `npm run test:backups-browser` (also part of `npm run test:mobile`) exercises the Backups section by touch at 360/390/desktop/short widths in both themes.
...
September 16, 2026 local validation: `npm run check` passed 192 tests in 40 files, Biome, application/runtime TypeScript and production build; `npm run test:backup` passed 21 isolated recovery tests. `npm run test:deploy-context` passed with 133 packaged inputs, including the recovery CLI. Independent bounded review resulted in regressions for replica/archive symlink aliasing, participant-owned `auth.sqlite` preservation, and pre-open writer-lock symlink rejection; no blocking review findings remained. Existing build chunk-size and experimental SQLite notices remain. No UI changed; no browser/device claims, live backup, cloud mutation, paid inference, push or deployment are implied by these local checks.
