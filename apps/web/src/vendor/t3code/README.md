...
| `ComposerPrimaryActions.tsx` | `apps/web/src/components/chat/ComposerPrimaryActions.tsx` | Original send/stop markup, sizes, colors and interactions, including upstream's pairing of the stop control with the send control while a turn runs (verified against the pinned file); that send control is Civic Spark's queue action for the running turn. Civic Spark state props replace T3 session state; plan and multi-question pending actions are omitted, and the pair is wrapped in upstream's `gap-1.5` row because this footer uses space-between. Upstream has no stopping state: a requested stop disables the stop control and shows the same upstream spinner until the server acknowledges it. |
