...
import { relayAgentBackend, relayTerminalBackend } from "./sessions.ts";
 ...
 * children (agent runners, terminal PTYs, helper sessions, one-shot commands);
 * the main process keeps HTTP, WebSocket clients, authorization, leases and
 * idle release. Returns undefined when the flag is 0 so callers fall back to
 * the in-process backends unchanged.
 */
